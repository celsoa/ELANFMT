...
%% Running on GPU
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
code_name    = 'FastWaveED_GPU3D_v1_16kmst'; %--compiler-options
...
figure(4),clf,
Max_rec = 2048*2;
isyn_c_nameZ = sprintf('16synseis.z.txt');isyn_cZ = load(isyn_c_nameZ);
delay = 1440;synseis_delay = zeros(1,nt);synseis_delay(delay+1:Max_rec+delay)=isyn_cZ(1: [Max_rec]) ;

isyn_c_nameX = sprintf('16synseis.r.txt');isyn_cX = load(isyn_c_nameX);
synseis_delayX = zeros(1,nt);synseis_delayX(delay+1:Max_rec+delay)=isyn_cX(1: [Max_rec]) ;

isyn_c_nameY = sprintf('16synseis.t.txt');isyn_cY = load(isyn_c_nameY);
...
start_num = 1000*0 + 1; start_num2 = 0*1000;
...
GPUF=1.5/3/1e-8*Rec2((start_num+start_num2):end-end_minus)*0.021;%1.002;
 
 flo = 1/25;%0.065;      % Hz
fhi = 1/12.5;%0.11;      % Hz
fs_Hz = 1/dt;% 1.0/dt; % sampling rate, Hz

signal = zeros(150000,1);signal(1:5500)=GPUF(1:5500);GPUF=signal;
signal = zeros(150000,1);signal(1:5500)=ZhuF(1:5500)';ZhuF=signal;
...
plot( Tsignal(1+5400*0 :end )/33.3407,-yG(1:end-5400*0),'b-.','LineWidth',1.5); title('Vertical'); 
 legend('Zhu and Rivera (2002)', 'Present study, FastWave GPU3D');% ylim([-5e-3 5e-3]); xlim([1 5.5]);   %Rec2(1000:end-end_minus)
 xlabel(' Time (s)'); ylabel('v (m/s)');  legend boxoff;ylim([-4.5e-5 4.5e-5]);
 ylabel('u (cm)');
 
subplot(1,3,2);% clf
ZhuF=(synseis_delayX(start_Zhu:end-end_Zhu));
GPUF=1.2/3/1e-8*(RecX((start_num+start_num2):end-end_minus))*0.036;%1.3;
...
plot(Tsignal/33.3407, yZ,'r-','LineWidth',1.5); hold on;
plot(Tsignal(1+500+2200*0:end)/33.3407,  yG(1:end-500+2200*0),'b-.','LineWidth',1.5); title('Radial'); 
 legend('Zhu and Rivera (2002)', 'Present study, FastWave GPU3D');%ylim([-2e-3 2e-3]); xlim([1 5.5]);  
 xlabel(' Time (s)'); ylabel('v (m/s)');  legend boxoff;%ylim([-3*1e-5 3*1e-5]);
 ylim([-4.5e-5 4.5e-5]);
% subplot(1,3,2); plot(x_timeZHU,(synseis_delayX(start_Zhu:end-end_Zhu)),'r-','LineWidth',3.0); hold on; 
% plot(x_timeGPU(1:end-start_num2),1/5.4/1e-7*(RecX((start_num+start_num2):end-end_minus)),'b-.','LineWidth',2.0); title('Radial'); 
% legend('Zhu and Rivera (2002)', 'Present study, FastWave GPU3D'); ylim([-4 4]); xlim([1 7.0]);  
% %hold on; plot((-Ampl.*Rec_x1(1000:end-end_minus)),'g-','LineWidth',2.0);
% xlabel(' Time (s)'); ylabel('v (m/s)');  legend boxoff
 
subplot(1,3,3); %clf
ZhuF=synseis_delayY(start_Zhu:end-end_Zhu);
GPUF=12/3/1e-7*RecY((start_num+start_num2):end-end_minus-0*20)*0.125/3;
 
 flo = 1/25;%0.065;      % Hz
fhi = 1/12.5;%0.11;      % Hz
fs_Hz = 1/dt;% 1.0/dt; % sampling rate, Hz

signal = zeros(150000,1);signal(1:7500)=GPUF(1:7500);GPUF=signal;
signal = zeros(150000,1);signal(1:7500)=ZhuF(1:7500)';ZhuF=signal;
Tsignal = 1*dt:1*dt:150000*dt;
flo = flo/(fs_Hz/2);
fhi = fhi/(fs_Hz/2);
...
plot(Tsignal(1+4000*0:end)/33.3407,yZ(1:end-4000*0),'r-','LineWidth',1.5); hold on;
plot(Tsignal/33.3407,yG,'b-.','LineWidth',1.5); title('Transverse'); 
 ...
 xlabel(' Time (s)'); ylabel('v (m/s)');  ylim([-4*1e-7 4*1e-7]);
 ylim([-4.5e-5 4.5e-5]);
legend boxoff
if 4>3; return; end
%%
...
start_num2 = 900;
plot(x_timeGPU(1:end-start_num2), 1/5.4/1e-7*Rec2((start_num+start_num2):end-end_minus),'b-.','LineWidth',2.0); title('Vertical'); 
legend('Zhu and Rivera (2002)', 'Present study, FastWave GPU3D'); ylim([-3 3]); xlim([1 7.0]);   %Rec2(1000:end-end_minus)
...
plot(x_timeGPU(1+50:end-start_num2),-1/5.4/1e-7*RecY((start_num+start_num2):end-end_minus-50),'b-.','LineWidth',2.0); title('Transverse'); 
legend('Zhu and Rivera (2002)', 'Present study, FastWave GPU3D'); ylim([-1.5 1.5]); xlim([1 7.0]);   
%hold on; plot(-Ampl.*Rec_y1(1000:end-end_minus),'g-','LineWidth',2.0);
xlabel(' Time (s)'); ylabel('v (m/s)'); 
legend boxoff

zzz = 1;
delete *.res *.inf  a.out
