...
%% 
% === Initialize logical inclusion mask ===
incl_mask = false(nx, ny, nz);  % Same shape as Cij arrays

% === Define inclusions in XZ plane at fixed y ===
n_inclusions = 750;
inclusion_size = 10;
buffer = 20;
zmin = layer1dz + buffer;
zmax = nz - buffer;

rho_incl   = 2700;
vrho_incl  = 1 / rho_incl;

rng(42);
x0 = randi([buffer, nx - inclusion_size - buffer], n_inclusions, 1);
z0 = randi([zmin, nz - inclusion_size - buffer], n_inclusions, 1);
y_fixed = round(ny / 2);  % center Y-plane

for i = 1:n_inclusions
    ix = x0(i):(x0(i) + inclusion_size - 1);
    iz = z0(i):(z0(i) + inclusion_size - 1);

    % Centered around y_fixed, clamp to [1, ny]
    y_start = max(1, y_fixed - floor(inclusion_size / 2));
    y_end   = min(ny, y_fixed + ceil(inclusion_size / 2) - 1);
    iy = y_start:y_end;

    % Update material properties
    vrho_x_11_h(ix, iy, iz) = vrho_incl;
    vrho_y_11_h(ix, iy, iz) = vrho_incl;
    vrho_z_11_h(ix, iy, iz) = vrho_incl;

    C11(ix, iy, iz) = 75e9;
    C22(ix, iy, iz) = 72e9;
    C33(ix, iy, iz) = 68e9;       % use 0 to mark inclusion
    C12(ix, iy, iz) = 32e9;
    C13(ix, iy, iz) = 27e9;
    C23(ix, iy, iz) = 25e9;
    C44(ix, iy, iz) = 28e9;
...
%% === 1. Compute voxel-centered coordinates ===
%% === 1. Compute voxel-centered coordinates ===
xv = linspace(-Lx/2 + dx/2, Lx/2 - dx/2, nx);
yv = linspace(-Ly/2 + dy/2, Ly/2 - dy/2, ny);
%zv = linspace(-Lz + dz/2, 0 - dz/2, nz);  % Z goes from -Lz (bottom) to 0 (surface)
zv = linspace(0 - dz/2, -Lz + dz/2, nz);  % Top (0) to bottom (-Lz)
[Y, X, Z] = meshgrid(yv, xv, zv);  % meshgrid uses (y,x,z) order!

%% === 2. Create mask for inclusions: C33 == 68e9 ===
inclusion_mask = (C33 == 68e9);  % Logical array
M = smooth3(double(inclusion_mask), 'box', 3);  % Optional smoothing

%% === 3. Visualize with isosurface ===
figure(101); clf
p = patch(isosurface(X, Y, Z, M, 0.5));
set(p, 'FaceColor', [1 0.1 0.1], 'EdgeColor', 'none', 'FaceAlpha', 0.6); hold on

xlabel('x (m)'); ylabel('y (m)'); zlabel('depth (m)');
title('3D Isosurface of Inclusions (Orthogneiss)');
%set(gca, 'ZDir', 'reverse');  % Flip Z-axis to show depth increasing downward

%% === 4. Manually draw a wireframe bounding box ===
xbox = [xv(1), xv(end)];
ybox = [yv(1), yv(end)];
zbox = [zv(1), zv(end)];   % might be decreasing
zlim(sort(zbox)); 

% Define 8 corners of the box
corners = [
    xbox(1) ybox(1) zbox(1);
    xbox(2) ybox(1) zbox(1);
    xbox(2) ybox(2) zbox(1);
    xbox(1) ybox(2) zbox(1);
    xbox(1) ybox(1) zbox(2);
    xbox(2) ybox(1) zbox(2);
    xbox(2) ybox(2) zbox(2);
    xbox(1) ybox(2) zbox(2);
];

% Define edges between corners (pairs of indices)
edges = [
    1 2; 2 3; 3 4; 4 1;
    5 6; 6 7; 7 8; 8 5;
    1 5; 2 6; 3 7; 4 8
...
for i = 1:size(edges, 1)
    ...
    plot3([pt1(1) pt2(1)], [pt1(2) pt2(2)], [pt1(3) pt2(3)], 'k-', 'LineWidth', 1.2);
end

%% === 5. Axis limits and appearance ===
xlim(xbox); ylim(ybox); zlim(sort(zbox)); 
daspect([1 1 1]);
grid on; box on
...
% --- Helper functions ---
function C = fill_layer(C1, C2)
    C = ones(nx, ny, nz) * C2;
    C(:,:,1:layer1dz) = C1;
end

function savecij(field, fname)
    fid = fopen(fname, 'wb'); fwrite(fid, field(:), 'single'); fclose(fid);
end