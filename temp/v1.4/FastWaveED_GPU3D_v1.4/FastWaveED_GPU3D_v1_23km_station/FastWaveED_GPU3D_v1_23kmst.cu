...
//const int nt     =  10;
const int nt     =  8500;//1600;//1600;//3550;//2550
const int nout   =  200; 
const int niter  =  10;
const int nsave  = 15;
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
__global__ void source(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz,DAT* sigma_xy,DAT* sigma_xz,DAT* sigma_yz, DAT* Vx, DAT* Vy, DAT* Qxft, DAT* Qyft,DAT* Src, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
    ...
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
    //-0.5689 - 0.5368    0.0986 23km
    //  - 0.5368    0.8844    0.3456
    // 0.0986    0.3456    0.2439
    // 
    //-0.7448 - 0.0387 - 0.1953 %% 16km
    //    - 0.0387    1.0603 - 0.3017
    //    - 0.1953 - 0.3017    0.2439
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_xx[ix + iy*nx + iz*nx*ny]  = sigma_xx[ix + iy*nx + iz*nx*ny]- (DAT)0.5689*Src[it]  ; }
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_yy[ix + iy*nx + iz*nx*ny]  = sigma_yy[ix + iy*nx + iz*nx*ny]+ (DAT)0.8844 *Src[it]  ; }
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_zz[ix + iy*nx + iz*nx*ny]  = sigma_zz[ix + iy*nx + iz*nx*ny]+ (DAT)0.2439*Src[it]  ; }//(DAT)1.0*
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+1) && iy>(ysrc+1) && ix<(xsrc+ds+1) && ix>(xsrc+1)){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]  = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] - (DAT)0.5368 *Src[it]  ; }
	//if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds-2) && iy>(ysrc-2) && ix<(xsrc+ds-2) && ix>(xsrc-2)){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]  = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] - (DAT)0.0332*Src[it]  ; }
	...
	//if (iz<(zsrc+ds+2) && iz>(zsrc+2) && iy<(ysrc+ds+2) && iy>(ysrc+2) && ix<(xsrc+ds+2) && ix>(xsrc+2)){ sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)]  = sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)] + (DAT)0.8*Src[it]  ; }
	//if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+2) && iy>(ysrc+2) && ix<(xsrc+ds+2) && ix>(xsrc+2)){ sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]  = sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] + (DAT)1.0*Src[it]  ; }
	...
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+1) && iy>(ysrc+1) && ix<(xsrc+ds+2) && ix>(xsrc+2)){ sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)]  = sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)] + (DAT)0.3456 *Src[it]  ; }
	...
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+0) && iy>(ysrc+0) && ix<(xsrc+ds-2) && ix>(xsrc-2)){ sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]  = sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] + (DAT)0.0986 *Src[it]  ; }
...
__global__ void record(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Rec,DAT* Rec_x,DAT* Rec_y,DAT* Rec_y2, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
	...
	//if (iz>0 && iz<2 && iy>384 && iy<386 && ix>503 && ix<505){ Rec[it]  = (DAT)0.5*Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )] + (DAT)0.5*Vz[ix+0+(iy+0)*(nx) +iz*(nx  )*(ny )]  ;} 
	//if (iz>0 && iz<2 && iy>384 && iy<386 && ix>503 && ix<505){ Rec_x[it]  = (DAT)0.5*Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] + (DAT)0.5*Vx[ix+(iy+0)*(nx+1)+ iz*(nx+1)*(ny  )]; }
	//if (iz>0 && iz<2 && iy>387 && iy<389 && ix>503 && ix<505){ Rec_y[it]  = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
	//if (iz>0 && iz<2 && iy>386 && iy<388 && ix>503 && ix<505){ Rec_y2[it] = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>1330 && ix< 1332){ Rec[it]  = (DAT)0.5*Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )] + (DAT)0.5*Vz[ix+0+(iy+0)*(nx) +iz*(nx  )*(ny )]  ;}
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>1330 && ix< 1332){ Rec_x[it]  = (DAT)0.5*Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] + (DAT)0.5*Vx[ix+(iy+0)*(nx+1)+ iz*(nx+1)*(ny  )]; }
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>1330 && ix< 1332){ Rec_y[it]  = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>1330 && ix< 1332){ Rec_y2[it] = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
}
...
__global__ void BC1(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Rec, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
    ...
    //if (iz==1 && iz<(nz-1) && iy>0 && iy<(ny-1) && ix>0 && ix<nx){        Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]   = (DAT)0.0;    }  
    //if (iz==1 && iz<(nz-1) && iy>0 && iy<ny && ix>0 && ix<(nx-1)){        Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   = (DAT)0.0;    } 
    //if (iz==1 && iz<nz && iy>0 && iy<(ny-1) && ix>0 && ix<(nx-1)){        Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]   = (DAT)0.0;    }
}

__global__ void compute_StressPrf(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft, DAT* c11u_het, DAT* c12u_het,DAT* c44u_hetxy,DAT* c44u_hetxz,DAT* c44u_hetyz,DAT* weights, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT c11u,const DAT c22u,const DAT c33u,const DAT c12u,const DAT c13u,const DAT c23u,const DAT c44u,const DAT c55u,const DAT c66u,const DAT alpha1,const DAT alpha2,const DAT alpha3,const DAT M1, const int nx, const int ny, const int nz){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
    ...
    if (iz<nz && iy<ny && ix<nx){
        ...
        DAT Vzz = diffVz;
        //DAT Qff = div_Qf;
        sigma_xx[ix + iy*nx + iz*nx*ny]     = sigma_xx[ix + iy*nx + iz*nx*ny]*weights[ix + iy*nx + iz*nx*ny] + dt*(c11u_het[ix + iy*nx + iz*nx*ny]* Vxx + c12u_het[ix + iy*nx + iz*nx*ny]* Vyy + c12u_het[ix + iy*nx + iz*nx*ny]* Vzz ); //+ alpha1*M1*Qff
        sigma_yy[ix + iy*nx + iz*nx*ny]     = sigma_yy[ix + iy*nx + iz*nx*ny]*weights[ix + iy*nx + iz*nx*ny] + dt*(c12u_het[ix + iy*nx + iz*nx*ny]* Vxx + c11u_het[ix + iy*nx + iz*nx*ny]* Vyy + c12u_het[ix + iy*nx + iz*nx*ny]* Vzz ); //+ alpha2*M1*Qff
        sigma_zz[ix + iy*nx + iz*nx*ny]     = sigma_zz[ix + iy*nx + iz*nx*ny]*weights[ix + iy*nx + iz*nx*ny] + dt*(c12u_het[ix + iy*nx + iz*nx*ny]* Vxx + c12u_het[ix + iy*nx + iz*nx*ny]* Vyy + c11u_het[ix + iy*nx + iz*nx*ny]* Vzz ); //+ alpha3*M1*Qff
        //Prf[ix + iy*nx + iz*nx*ny] = Prf[ix + iy*nx + iz*nx*ny] + dt*(  -alpha1* M1* Vxx  - alpha2* M1* Vyy - alpha3* M1* Vzz - M1* Qff );
    }
    if (iz<nz && iy>0 && iy<ny && ix>0 && ix<nx){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]*weights[ix + iy*nx + iz*nx*ny] + c44u_hetxy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]*dt*( (Vy[ix + iy* nx + iz*nx  *(ny+1)] - Vy[ix-1 + iy* nx + iz*nx*(ny+1)  ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)*(ny-0)] - Vx[ix  +(iy-1)*(nx+1) +  iz   *(nx+1)*ny])*((DAT)1.0/dy) ); }

    if (iz>0 && iz<nz && iy<ny && ix>0 && ix<nx){ sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] = sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]*weights[ix + iy*nx + iz*nx*ny] + c44u_hetxz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]*dt*( (Vz[ix + iy* nx + iz*nx  * ny   ] - Vz[ix-1 + iy* nx + iz*nx* ny     ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)* ny   ] - Vx[ix + iy* (nx+1)    + (iz-1)*(nx+1)*ny])*((DAT)1.0/dz) ); }

    if (iz>0 && iz<nz && iy>0 && iy<ny && ix<nx){ sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)] = sigma_yz[ix + iy* nx    + iz*nx*(ny+1)    ]*weights[ix + iy*nx + iz*nx*ny] + c44u_hetyz[ix + iy* nx    + iz*nx*(ny+1)    ]*dt*( (Vy[ix + iy* nx + iz*nx*  (ny+1)] - Vy[ix + iy* nx + (iz-1)*nx*(ny+1)])*((DAT)1.0/dz) + (Vz[ix + iy* nx    + iz*nx*ny        ] - Vz[ix + (iy-1)* nx    +  iz   * nx   *ny])*((DAT)1.0/dy) ); }
...
__global__ void update_Qxft(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft,DAT* Qxold,DAT* Qyold,DAT* Qzold,DAT* vrhox11het,DAT* vrhoy11het,DAT* vrhoz11het,DAT* weights, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT eta_k1,const DAT eta_k2,const DAT eta_k3,const DAT vrho_x_11, const DAT vrho_y_11,const DAT vrho_z_11,const DAT vrho_x_12,const DAT vrho_y_12,const DAT vrho_z_12,const DAT vrho_x_22,const DAT vrho_y_22,const DAT vrho_z_22, const int nx, const int ny, const int nz, const DAT chi){
    ...
    if (iz>0 && iz<(nz-1) && iy>0 && iy<(ny-1) && ix>0 && ix<nx){//vrho_x_11 (nx+1)
        ...
        Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]   = ( Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhox11het[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] * dSx  )*dt - (DAT)2.5* (DAT)4.6729e-04 * Vx[ix + iy * (nx + 1) + iz * (nx + 1) * (ny)];
    }   //+ vrho_x_12* ( QPx + chi*eta_k1*Qxft[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] ) + vrho_x_11    vrhox11het[(ix-1) + (iy  )*(nx-1) + (iz  )*(nx-1)*ny]
    if (iz>0 && iz<(nz-1) && iy>0 && iy<ny && ix>0 && ix<(nx-1)){
        ...
        Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   = ( Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhoy11het[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]* dSy   )*dt - (DAT)2.5* (DAT)4.6729e-04 * Vy[ix + iy * (nx)+iz * (nx) * (ny + 1)];
    } //+ vrho_y_12* ( QPy + chi*eta_k2*Qyft[ix + iy*(nx  )   + iz*(nx  )*(ny+1)] )  vrho_y_11     vrhoy11het[(ix ) + (iy-1  )*(nx) + (iz  )*(nx)*(ny-1)]
    if (iz>0 && iz<nz && iy>0 && iy<(ny-1) && ix>0 && ix<(nx-1)){
        ...
        Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]   = ( Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhoz11het[ix + iy*(nx  ) + iz*(nx  )*(ny  )]* dSz   )*dt - (DAT)2.5* (DAT)4.6729e-04 * Vz[ix + iy * (nx)+iz * (nx) * (ny)];
    } //+ vrho_z_12* ( QPz + chi*eta_k3*Qzft[ix + iy*(nx  )   + iz*(nx  )*(ny  )] ) vrho_z_11     vrhoz11het[(ix) + (iy  )*(nx) + (iz-1)*(nx)*ny]
...
int main(){
    ...
    //double eta_k1=pa3_h[0],eta_k2=pa3_h[1],eta_k3=pa3_h[2],mm1=pa3_h[3],mm2=pa3_h[4],mm3=pa3_h[5],delta1=pa3_h[6],delta2 =pa3_h[7],delta3=pa3_h[8],rho_fluid1=pa3_h[9],rho_fluid2=pa3_h[10],rho_fluid3 =pa3_h[11],rho1=pa3_h[12],rho2=pa3_h[13],rho3=pa3_h[14];
     double vrho_x_11=pa3_h[0],vrho_y_11=pa3_h[1],vrho_z_11=pa3_h[2],vrho_x_12=pa3_h[3],vrho_y_12=pa3_h[4],vrho_z_12=pa3_h[5],vrho_x_22=pa3_h[6],vrho_y_22=pa3_h[7],vrho_z_22=pa3_h[8], eta_k1=pa3_h[9],eta_k2=pa3_h[10],eta_k3=pa3_h[11];
    const DAT chi    = (DAT)0.5;

load3(vrhox11het,nx+1 , ny,nz,"vrho_x_11_hc.dat");
def_sizes(vrhox11het,nx+1,ny,nz); 
load3(vrhoy11het,nx , ny+1,nz,"vrho_y_11_hc.dat");
def_sizes(vrhoy11het,nx,ny+1,nz); 
load3(vrhoz11het,nx , ny,nz+1,"vrho_z_11_hc.dat");
def_sizes(vrhoz11het,nx,ny,nz+1); 
...
load3(c44u_hetxy,nx+1 , ny+1,nz,"c44u_hetxy.dat");
def_sizes(c44u_hetxy,nx+1,ny+1,nz);
load3(c44u_hetxz,nx+1 , ny,nz+1,"c44u_hetxz.dat");
def_sizes(c44u_hetxz,nx+1,ny,nz+1);
load3(c44u_hetyz,nx , ny+1,nz+1,"c44u_hetyz.dat");
def_sizes(c44u_hetyz,nx,ny+1,nz+1);

load3(weights,nx , ny,nz,"weights.dat");
def_sizes(weights,nx,ny,nz); 
    ...
    int xsrc = (int)180;//round(x_s/dx); 234
    int ysrc = (int)round(y_s/dy);
    int zsrc = (int)200;//round(z_s/dz);
    //zeros(Src ,nt,1,1);
...
printf("\nysrc = %10.8d",ysrc);
    ...
    for (it=0;it<nt;it++){
        //if (it==51){ tic(); }
        compute_StressPrf<<<grid,block>>>(Prf_d, sigma_xx_d, sigma_yy_d,sigma_zz_d, sigma_xy_d, sigma_xz_d, sigma_yz_d, Vx_d, Vy_d,Vz_d, Qxft_d, Qyft_d,Qzft_d, c11u_het_d,c12u_het_d,c44u_hetxy_d,c44u_hetxz_d,c44u_hetyz_d,weights_d, dx, dy, dz, dt, c11u,c22u, c33u, c12u,c13u,c23u,         c44u,c55u,c66u, alpha1,alpha2, alpha3, M1, nx, ny, nz);  
...
source<<<grid,block>>>(x_d, y_d, z_d, Prf_d, sigma_xx_d, sigma_yy_d, sigma_zz_d, sigma_xy_d,sigma_xz_d, sigma_yz_d, Vx_d, Vy_d, Qxft_d, Qyft_d, Src_d, dx, dy,dz, Lx, Ly,Lz,lamx, lamy,lamz, nx, ny, nz, it, xsrc, ysrc, zsrc, ds,dt); cudaDeviceSynchronize();
        ...
        update_Qxft<<<grid,block>>>(Prf_d, sigma_xx_d, sigma_yy_d,sigma_zz_d, sigma_xy_d, sigma_xz_d, sigma_yz_d, Vx_d, Vy_d,Vz_d, Qxft_d, Qyft_d,Qzft_d,Qxold_d,Qyold_d,Qzold_d,vrhox11het_d,vrhoy11het_d,vrhoz11het_d,weights_d, dx, dy,dz, dt,eta_k1, eta_k2,eta_k3, vrho_x_11, vrho_y_11,vrho_z_11, vrho_x_12,vrho_y_12, vrho_z_12, vrho_x_22,vrho_y_22, vrho_z_22, nx, ny, nz, chi);
        cudaDeviceSynchronize();
