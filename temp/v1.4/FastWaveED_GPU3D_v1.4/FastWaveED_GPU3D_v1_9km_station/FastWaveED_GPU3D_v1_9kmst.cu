...
//const int nt     =  10;
const int nt     =  4500;//1600;//1600;//3550;//2550
const int nout   =  200; 
const int niter  =  10;
...
__global__ void source(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz,DAT* sigma_xy,DAT* sigma_xz,DAT* sigma_yz, DAT* Vx, DAT* Vy, DAT* Qxft, DAT* Qyft,DAT* Src, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
    ...
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_xx[ix + iy*nx + iz*nx*ny]  = sigma_xx[ix + iy*nx + iz*nx*ny]- (DAT)0.6282*Src[it]  ; } 
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_yy[ix + iy*nx + iz*nx*ny]  = sigma_yy[ix + iy*nx + iz*nx*ny]+ (DAT)0.9437*Src[it]  ; }
	...
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+1) && iy>(ysrc+1) && ix<(xsrc+ds+1) && ix>(xsrc+1)){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]  = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] + (DAT)0.4455*Src[it]  ; }
	...
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+1) && iy>(ysrc+1) && ix<(xsrc+ds+2) && ix>(xsrc+2)){ sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)]  = sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)] - (DAT)0.2362*Src[it]  ; }
	...
	if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+0) && iy>(ysrc+0) && ix<(xsrc+ds-2) && ix>(xsrc-2)){ sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]  = sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] - (DAT)0.2709*Src[it]  ; }
...
__global__ void record(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Rec,DAT* Rec_x,DAT* Rec_y,DAT* Rec_y2, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
	...
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>630 && ix<632){ Rec[it]  = (DAT)0.5*Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )] + (DAT)0.5*Vz[ix+0+(iy+0)*(nx) +iz*(nx  )*(ny )]  ;} 
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>630 && ix<632){ Rec_x[it]  = (DAT)0.5*Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] + (DAT)0.5*Vx[ix+(iy+0)*(nx+1)+ iz*(nx+1)*(ny  )]; }
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>630 && ix<632){ Rec_y[it]  = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
	if (iz>0 && iz<2 && iy>256 && iy<258 && ix>630 && ix<632){ Rec_y2[it] = Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   ; }
}
...
__global__ void update_Qxft(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft,DAT* Qxold,DAT* Qyold,DAT* Qzold,DAT* vrhox11het,DAT* vrhoy11het,DAT* vrhoz11het,DAT* weights, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT eta_k1,const DAT eta_k2,const DAT eta_k3,const DAT vrho_x_11, const DAT vrho_y_11,const DAT vrho_z_11,const DAT vrho_x_12,const DAT vrho_y_12,const DAT vrho_z_12,const DAT vrho_x_22,const DAT vrho_y_22,const DAT vrho_z_22, const int nx, const int ny, const int nz, const DAT chi){
    ...
    if (iz>0 && iz<(nz-1) && iy>0 && iy<(ny-1) && ix>0 && ix<nx){//vrho_x_11 (nx+1)
        ...
        Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]   = ( Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhox11het[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] * dSx  )*dt;
    ...
    if (iz>0 && iz<(nz-1) && iy>0 && iy<ny && ix>0 && ix<(nx-1)){
        ...
        Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   = ( Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhoy11het[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]* dSy   )*dt;
    ...
    if (iz>0 && iz<nz && iy>0 && iy<(ny-1) && ix>0 && ix<(nx-1)){
        ...
        Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]   = ( Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]*((DAT)1.0/dt)*weights[ix + iy*nx + iz*nx*ny] + vrhoz11het[ix + iy*(nx  ) + iz*(nx  )*(ny  )]* dSz   )*dt;
