...
__global__ void compute_StressOrtho(
    DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz,
    DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz,
    DAT* Vx, DAT* Vy, DAT* Vz,
    DAT* c11u_het, DAT* c22u_het, DAT* c33u_het,
    DAT* c12u_het, DAT* c13u_het, DAT* c23u_het,
    DAT* c66u_hetxy, DAT* c55u_hetxz, DAT* c44u_hetyz,
    DAT* weights,
    const DAT dx, const DAT dy, const DAT dz, const DAT dt,
    const int nx, const int ny, const int nz)
{
    ...
    if (ix < nx && iy < ny && iz < nz) {
        ...
        sigma_xx[idx] = sigma_xx[idx] * weights[idx] + dt * (
            c11u_het[idx] * Vxx
            + c12u_het[idx] * Vyy
            + c13u_het[idx] * Vzz);

        sigma_yy[idx] = sigma_yy[idx] * weights[idx] + dt * (
            c12u_het[idx] * Vxx
            + c22u_het[idx] * Vyy
            + c23u_het[idx] * Vzz);

        sigma_zz[idx] = sigma_zz[idx] * weights[idx] + dt * (
            c13u_het[idx] * Vxx
            + c23u_het[idx] * Vyy
            + c33u_het[idx] * Vzz);
    }
    if (ix > 0 && ix < nx && iy > 0 && iy < ny && iz < nz) {
        int idx_xy = ix + iy * (nx + 1) + iz * (nx + 1) * (ny + 1);
        int idx = ix + iy * nx + iz * nx * ny;
        DAT dVx_dy = (Vx[ix + iy * (nx + 1) + iz * (nx + 1) * ny] - Vx[ix + (iy - 1) * (nx + 1) + iz * (nx + 1) * ny]) / dy;
        DAT dVy_dx = (Vy[ix + iy * nx + iz * nx * (ny + 1)] - Vy[ix - 1 + iy * nx + iz * nx * (ny + 1)]) / dx;

        sigma_xy[idx_xy] = sigma_xy[idx_xy] * weights[idx] + dt * c66u_hetxy[idx_xy] * (dVy_dx + dVx_dy);
    }

    if (ix > 0 && ix < nx && iz > 0 && iz < nz && iy < ny) {
        int idx_xz = ix + iy * (nx + 1) + iz * (nx + 1) * ny;
        int idx = ix + iy * nx + iz * nx * ny;
        DAT dVx_dz = (Vx[ix + iy * (nx + 1) + iz * (nx + 1) * ny] - Vx[ix + iy * (nx + 1) + (iz - 1) * (nx + 1) * ny]) / dz;
        DAT dVz_dx = (Vz[ix + iy * nx + iz * nx * ny] - Vz[ix - 1 + iy * nx + iz * nx * ny]) / dx;

        sigma_xz[idx_xz] = sigma_xz[idx_xz] * weights[idx] + dt * c55u_hetxz[idx_xz] * (dVz_dx + dVx_dz);
    }

    if (iy > 0 && iy < ny && iz > 0 && iz < nz && ix < nx) {
        int idx_yz = ix + iy * nx + iz * nx * (ny + 1);
        int idx = ix + iy * nx + iz * nx * ny;
        DAT dVy_dz = (Vy[ix + iy * nx + iz * nx * (ny + 1)] - Vy[ix + iy * nx + (iz - 1) * nx * (ny + 1)]) / dz;
        DAT dVz_dy = (Vz[ix + iy * nx + iz * nx * ny] - Vz[ix + (iy - 1) * nx + iz * nx * ny]) / dy;

        sigma_yz[idx_yz] = sigma_yz[idx_yz] * weights[idx] + dt * c44u_hetyz[idx_yz] * (dVy_dz + dVz_dy);
    }
    #undef diffVx
    #undef diffVy
    #undef diffVz
...
__global__ void update_VelocityFromStress(
    DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz,
    DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz,
    DAT* Vx, DAT* Vy, DAT* Vz,
    DAT* vrhox11het, DAT* vrhoy11het, DAT* vrhoz11het,
    DAT* weights,
    const DAT dx, const DAT dy, const DAT dz, const DAT dt,
    const int nx, const int ny, const int nz){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
...
load3(c11u_het, nx, ny, nz, "C11m.dat");
def_sizes(c11u_het, nx, ny, nz);

load3(c22u_het, nx, ny, nz, "C22m.dat");
def_sizes(c22u_het, nx, ny, nz);

load3(c33u_het, nx, ny, nz, "C33m.dat");
def_sizes(c33u_het, nx, ny, nz);

load3(c12u_het, nx, ny, nz, "C12m.dat");
def_sizes(c12u_het, nx, ny, nz);

load3(c13u_het, nx, ny, nz, "C13m.dat");
def_sizes(c13u_het, nx, ny, nz);

load3(c23u_het, nx, ny, nz, "C23m.dat");
def_sizes(c23u_het, nx, ny, nz);

load3(c44u_hetyz, nx, ny + 1, nz + 1, "C44myz.dat"); // C44 = mu_yz
def_sizes(c44u_hetyz, nx, ny + 1, nz + 1);

load3(c55u_hetxz, nx + 1, ny, nz + 1, "C55mxz.dat"); // C55 = mu_xz
def_sizes(c55u_hetxz, nx + 1, ny, nz + 1);

load3(c66u_hetxy, nx + 1, ny + 1, nz, "C66mxy.dat"); // C66 = mu_xy
def_sizes(c66u_hetxy, nx + 1, ny + 1, nz);
...
printf("\nysrc = %10.8d",ysrc);
    ...
    for (it=0;it<nt;it++){
        //if (it==51){ tic(); }
        compute_StressOrtho << <grid, block >> > (
            sigma_xx_d, sigma_yy_d, sigma_zz_d,
            sigma_xy_d, sigma_xz_d, sigma_yz_d,
            Vx_d, Vy_d, Vz_d,
            c11u_het_d, c22u_het_d, c33u_het_d,
            c12u_het_d, c13u_het_d, c23u_het_d,
            c66u_hetxy_d, c55u_hetxz_d, c44u_hetyz_d,
            weights_d,
            dx, dy, dz, dt,
            nx, ny, nz); 
...
update_VelocityFromStress << <grid, block >> > (
    sigma_xx_d, sigma_yy_d, sigma_zz_d,
    sigma_xy_d, sigma_xz_d, sigma_yz_d,
    Vx_d, Vy_d, Vz_d,
    vrhox11het_d, vrhoy11het_d, vrhoz11het_d,
    weights_d,
    dx, dy, dz, dt,
    nx, ny, nz); 
cudaDeviceSynchronize();
