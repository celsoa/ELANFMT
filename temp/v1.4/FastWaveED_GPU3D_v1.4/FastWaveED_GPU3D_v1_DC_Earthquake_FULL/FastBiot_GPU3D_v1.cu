...
__global__ void compute_StressPrf(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft, DAT* c11u_het, DAT* c12u_het,DAT* weights, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT c11u,const DAT c22u,const DAT c33u,const DAT c12u,const DAT c13u,const DAT c23u,const DAT c44u,const DAT c55u,const DAT c66u,const DAT alpha1,const DAT alpha2,const DAT alpha3,const DAT M1, const int nx, const int ny, const int nz){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
    ...
    if (iz<nz && iy>0 && iy<ny && ix>0 && ix<nx){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]*weights[ix + iy*nx + iz*nx*ny] + c66u*dt*( (Vy[ix + iy* nx + iz*nx  *(ny+1)] - Vy[ix-1 + iy* nx + iz*nx*(ny+1)  ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)*(ny-0)] - Vx[ix  +(iy-1)*(nx+1) +  iz   *(nx+1)*ny])*((DAT)1.0/dy) ); }
    if (iz>0 && iz<nz && iy<ny && ix>0 && ix<nx){ sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] = sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ]*weights[ix + iy*nx + iz*nx*ny] + c55u*dt*( (Vz[ix + iy* nx + iz*nx  * ny   ] - Vz[ix-1 + iy* nx + iz*nx* ny     ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)* ny   ] - Vx[ix + iy* (nx+1)    + (iz-1)*(nx+1)*ny])*((DAT)1.0/dz) ); }
...
def_sizes(Src,nt,1,1); 
    ...
    for (it=0;it<nt;it++){
        //if (it==51){ tic(); }
        compute_StressPrf<<<grid,block>>>(Prf_d, sigma_xx_d, sigma_yy_d,sigma_zz_d, sigma_xy_d, sigma_xz_d, sigma_yz_d, Vx_d, Vy_d,Vz_d, Qxft_d, Qyft_d,Qzft_d, c11u_het_d,c12u_het_d,weights_d, dx, dy, dz, dt, c11u,c22u, c33u, c12u,c13u,c23u,         c44u,c55u,c66u, alpha1,alpha2, alpha3, M1, nx, ny, nz);  
