...
__global__ void source(DAT* x, DAT* y,DAT* z, DAT* Prf, DAT* sigma_xx, DAT* sigma_yy, DAT* sigma_zz,DAT* sigma_xy,DAT* sigma_xz,DAT* sigma_yz, DAT* Vx, DAT* Vy, DAT* Qxft, DAT* Qyft,DAT* Src, const DAT dx, const DAT dy,const DAT dz, const DAT Lx, const DAT Ly,const DAT Lz, const DAT lamx, const DAT lamy,const DAT lamz, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT dt){
    ...
    //if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_xx[ix + iy*nx + iz*nx*ny]  = sigma_xx[ix + iy*nx + iz*nx*ny]- (DAT)0.5*Src[it]  ; } 
    //if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_yy[ix + iy*nx + iz*nx*ny]  = sigma_yy[ix + iy*nx + iz*nx*ny]+ Src[it]  ; }
    //if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_zz[ix + iy*nx + iz*nx*ny]  = sigma_zz[ix + iy*nx + iz*nx*ny]- (DAT)0.25*Src[it]  ; }//(DAT)1.0*
...
__global__ void compute_StressPrf(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft, DAT* c11u_het, DAT* c12u_het, DAT* weights_x , DAT* weights_y , DAT* weights_z , DAT* Rec, DAT* Rec_x, DAT* Rec_y, DAT* Rec_y2, DAT* Src, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT c11u,const DAT c22u,const DAT c33u,const DAT c12u,const DAT c13u,const DAT c23u, DAT c44u,const DAT c55u,const DAT c66u,const DAT alpha1,const DAT alpha2,const DAT alpha3,const DAT M1, const int nx, const int ny, const int nz, int it, int xsrc, int ysrc, int zsrc, int ds, DAT MTSxx, DAT MTSyy, DAT MTSzz, DAT MTSxy, DAT MTSxz, DAT MTSyz){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
    ...
    if (iz<nz && iy<ny && ix<nx){
        ...
        DAT Vyy = diffVy;
        DAT Vzz = diffVz;
        //DAT wav_x = (DAT)0.5 * (weights_x[ix + 1] + weights_x[ix]);
        //DAT wav_y = (DAT)0.5 * (weights_y[iy + 1] + weights_y[iy]);
        //DAT wav_z = (DAT)0.5 * (weights_z[iz + 1] + weights_z[iz]);
        //DAT wa_x = (weights_x[ix - 1 + 0]);
        //DAT wa_y = (weights_y[iy - 1 + 0]);
        //DAT wa_z = (weights_z[iz - 1 + 0]);
        //DAT Qff = div_Qf;
        sigma_xx[ix + iy*nx + iz*nx*ny]     = (DAT)0.5 * (weights_x[ix + 1] + weights_x[ix]) * (sigma_xx[ix + iy*nx + iz*nx*ny] + dt*(c11u_het[ix + iy*nx + iz*nx*ny]* Vxx + c12u_het[ix + iy*nx + iz*nx*ny]* Vyy + c12u_het[ix + iy*nx + iz*nx*ny]* Vzz )); //+ alpha1*M1*Qff
        sigma_yy[ix + iy*nx + iz*nx*ny]     = (DAT)0.5 * (weights_y[iy + 1] + weights_y[iy]) * (sigma_yy[ix + iy*nx + iz*nx*ny] + dt*(c12u_het[ix + iy*nx + iz*nx*ny]* Vxx + c11u_het[ix + iy*nx + iz*nx*ny]* Vyy + c12u_het[ix + iy*nx + iz*nx*ny]* Vzz )); //+ alpha2*M1*Qff
        sigma_zz[ix + iy*nx + iz*nx*ny]     = (DAT)0.5 * (weights_z[iz + 1] + weights_z[iz]) * (sigma_zz[ix + iy*nx + iz*nx*ny] + dt*(c12u_het[ix + iy*nx + iz*nx*ny]* Vxx + c12u_het[ix + iy*nx + iz*nx*ny]* Vyy + c11u_het[ix + iy*nx + iz*nx*ny]* Vzz )); //+ alpha3*M1*Qff
        //Prf[ix + iy*nx + iz*nx*ny] = Prf[ix + iy*nx + iz*nx*ny] + dt*(  -alpha1* M1* Vxx  - alpha2* M1* Vyy - alpha3* M1* Vzz - M1* Qff );
        //*weights[ix + iy*nx + iz*nx*ny]
        //*weights[ix + iy*nx + iz*nx*ny]
        //*weights[ix + iy*nx + iz*nx*ny]
    ...
    //if (iz<nz && iy>0 && iy<ny && ix>0 && ix<nx){ 
        sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] = (weights_z[iz   + 0]) * (weights_y[iy   + 0]) * (weights_x[ix  + 0]) * (sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] + c66u*dt*( (Vy[ix + iy* nx + iz*nx  *(ny+1)] - Vy[ix-1 + iy* nx + iz*nx*(ny+1)  ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)*(ny-0)] - Vx[ix  +(iy-1)*(nx+1) +  iz   *(nx+1)*ny])*((DAT)1.0/dy) ));
    //}
    //if (iz>0 && iz<nz && iy<ny && ix>0 && ix<nx){ 
        sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] = (weights_y[iy   + 0]) * (weights_z[iz  + 0]) * (weights_x[ix   + 0]) * (sigma_xz[ix + iy*(nx+1) + iz*(nx+1)* ny   ] + c66u*dt*( (Vz[ix + iy* nx + iz*nx  * ny   ] - Vz[ix-1 + iy* nx + iz*nx* ny     ])*((DAT)1.0/dx) + (Vx[ix + iy* (nx+1)+ iz*(nx+1)* ny   ] - Vx[ix + iy* (nx+1)    + (iz-1)*(nx+1)*ny])*((DAT)1.0/dz) ));
    //}
    //if (iz>0 && iz<nz && iy>0 && iy<ny && ix<nx){ 
        sigma_yz[ix + iy* nx    + iz* nx   *(ny+1)] = (weights_x[ix   + 0]) * (weights_y[iy   + 0]) * (weights_z[iz   + 0]) * (sigma_yz[ix + iy* nx    + iz*nx*(ny+1)    ] + c66u*dt*( (Vy[ix + iy* nx + iz*nx*  (ny+1)] - Vy[ix + iy* nx + (iz-1)*nx*(ny+1)])*((DAT)1.0/dz) + (Vz[ix + iy* nx    + iz*nx*ny        ] - Vz[ix + (iy-1)* nx    +  iz   * nx   *ny])*((DAT)1.0/dy) ));
    //}
    }
    #undef diffVx //*weights[ix + iy*nx + iz*nx*ny]
    #undef diffVy //*weights[ix + iy*nx + iz*nx*ny]
    #undef diffVz //*weights[ix + iy*nx + iz*nx*ny]
    
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_xx[ix + iy*nx + iz*nx*ny]  = sigma_xx[ix + iy*nx + iz*nx*ny]+ MTSxx*Src[it]  ; } 
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_yy[ix + iy*nx + iz*nx*ny]  = sigma_yy[ix + iy*nx + iz*nx*ny]+ MTSyy*Src[it]  ; }
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds) && iy>(ysrc) && ix<(xsrc+ds) && ix>(xsrc)){ sigma_zz[ix + iy*nx + iz*nx*ny]  = sigma_zz[ix + iy*nx + iz*nx*ny]+ MTSzz*Src[it]  ; } 

    if (iz<(zsrc + ds) && iz>(zsrc) && iy<(ysrc + ds + 1) && iy>(ysrc + 1) && ix<(xsrc + ds + 1) && ix>(xsrc + 1)) { sigma_yz[ix + iy * nx + iz * nx * (ny + 1)] = sigma_yz[ix + iy * nx + iz * nx * (ny + 1)] + MTSyz * Src[it]; } //0.8
    if (iz<(zsrc + ds) && iz>(zsrc) && iy<(ysrc + ds + 2) && iy>(ysrc + 2) && ix<(xsrc + ds + 2) && ix>(xsrc + 2)) { sigma_xz[ix + iy * (nx + 1) + iz * (nx + 1) * ny] = sigma_xz[ix + iy * (nx + 1) + iz * (nx + 1) * ny] + MTSxz * Src[it]; } //1
    if (iz<(zsrc+ds) && iz>(zsrc) && iy<(ysrc+ds+2) && iy>(ysrc+2) && ix<(xsrc+ds+2) && ix>(xsrc+2)){ sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)]  = sigma_xy[ix + iy*(nx+1) + iz*(nx+1)*(ny+1)] + MTSxy *Src[it]  ; }

}

__global__ void update_Qxft(DAT* Prf, DAT* sigma_xx, DAT* sigma_yy,DAT* sigma_zz, DAT* sigma_xy, DAT* sigma_xz, DAT* sigma_yz, DAT* Vx, DAT* Vy,DAT* Vz, DAT* Qxft, DAT* Qyft,DAT* Qzft,DAT* Qxold,DAT* Qyold,DAT* Qzold,DAT* vrhox11het, DAT* weights_x , DAT* weights_y , DAT* weights_z , DAT* Rec, DAT* Rec_x, DAT* Rec_y, DAT* Rec_y2, const DAT dx, const DAT dy,const DAT dz,const DAT dt,const DAT eta_k1,const DAT eta_k2,const DAT eta_k3,const DAT vrho_x_11, const DAT vrho_y_11,const DAT vrho_z_11,const DAT vrho_x_12,const DAT vrho_y_12,const DAT vrho_z_12,const DAT vrho_x_22,const DAT vrho_y_22,const DAT vrho_z_22, const int nx, const int ny, const int nz, const DAT chi, int it){
    int ix = blockIdx.x*blockDim.x + threadIdx.x; // thread ID, dimension x
    int iy = blockIdx.y*blockDim.y + threadIdx.y; // thread ID, dimension y
    int iz = blockIdx.z*blockDim.z + threadIdx.z; // thread ID, dimension z
    ...
    if (iz>0 && iz<(nz-1) && iy>0 && iy<(ny-1) && ix>0 && ix<nx){//vrho_x_11 (nx+1)
        ...
        Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]   = w_x *( Vx[ix + iy*(nx+1) + iz*(nx+1)*(ny  )]*((DAT)1.0/dt) + vrho_x_11 * dSx  )*dt;
    }   //+ vrho_x_12* ( QPx + chi*eta_k1*Qxft[ix + iy*(nx+1) + iz*(nx+1)*(ny  )] )     vrhox11het[(ix-1) + (iy  )*(nx-1) + (iz  )*(nx-1)*ny] *weights[ix + iy*nx + iz*nx*ny]
    if (iz>0 && iz<(nz-1) && iy>0 && iy<ny && ix>0 && ix<(nx-1)){
        //DAT QPy = Q_gradPrfy;
        DAT dSy = div_Sigmay;
        DAT w_y = weights_y[iy - 1];
        ...
        Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]   = w_y * ( Vy[ix + iy*(nx  ) + iz*(nx  )*(ny+1)]*((DAT)1.0/dt) + vrho_y_11* dSy   )*dt;
    } //+ vrho_y_12* ( QPy + chi*eta_k2*Qyft[ix + iy*(nx  )   + iz*(nx  )*(ny+1)] )*weights[ix + iy*nx + iz*nx*ny]
    if (iz>0 && iz<nz && iy>0 && iy<(ny-1) && ix>0 && ix<(nx-1)){
        //DAT QPz = Q_gradPrfz;
        DAT dSz = div_Sigmaz;
        DAT w_z = weights_z[iz - 1];
        ...
        Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]   = w_z * ( Vz[ix + iy*(nx  ) + iz*(nx  )*(ny  )]*((DAT)1.0/dt) + vrho_z_11* dSz   )*dt;
    } //+ vrho_z_12* ( QPz + chi*eta_k3*Qzft[ix + iy*(nx  )   + iz*(nx  )*(ny  )] )*weights[ix + iy*nx + iz*nx*ny]
...
int main(){
    ...
    load(MTS, 6, 1, "MTS.dat")
    DAT MTSxx = MTS_h[0], MTSyy = MTS_h[1], MTSzz = MTS_h[2], MTSxy = MTS_h[3], MTSxz = MTS_h[4], MTSyz = MTS_h[5];
...
load3(weights_x,nx + 1, 1,1,"weights_x.dat");
def_sizes(weights_x,nx+1,1,1);
load3(weights_y, 1, ny + 1, 1, "weights_y.dat");
def_sizes(weights_y, 1, ny + 1, 1);
load3(weights_z, 1, 1, nz + 1, "weights_z.dat");
def_sizes(weights_z, 1, 1, nz + 1);
...
def_sizes(Src,nt,1,1); 
    ...
    for (it=0;it<nt;it++){
        //if (it==51){ tic(); }
        compute_StressPrf<<<grid,block>>>(Prf_d, sigma_xx_d, sigma_yy_d,sigma_zz_d, sigma_xy_d, sigma_xz_d, sigma_yz_d, Vx_d, Vy_d,Vz_d, Qxft_d, Qyft_d,Qzft_d, c11u_het_d,c12u_het_d,weights_x_d, weights_y_d, weights_z_d, Rec_d, Rec_x_d, Rec_y_d, Rec_y2_d, Src_d, dx, dy, dz, dt, c11u,c22u, c33u, c12u,c13u,c23u,         c44u,c55u,c66u, alpha1,alpha2, alpha3, M1, nx, ny, nz, it,  xsrc,  ysrc,  zsrc,  ds,   MTSxx,   MTSyy,   MTSzz,   MTSxy,   MTSxz,   MTSyz);
...
//source<<<grid,block>>>(x_d, y_d, z_d, Prf_d, sigma_xx_d, sigma_yy_d, sigma_zz_d, sigma_xy_d,sigma_xz_d, sigma_yz_d, Vx_d, Vy_d, Qxft_d, Qyft_d, Src_d, dx, dy,dz, Lx, Ly,Lz,lamx, lamy,lamz, nx, ny, nz, it, xsrc, ysrc, zsrc, ds,dt); cudaDeviceSynchronize();
        ...
        update_Qxft<<<grid,block>>>(Prf_d, sigma_xx_d, sigma_yy_d,sigma_zz_d, sigma_xy_d, sigma_xz_d, sigma_yz_d, Vx_d, Vy_d,Vz_d, Qxft_d, Qyft_d,Qzft_d,Qxold_d,Qyold_d,Qzold_d,vrhox11het_d, weights_x_d, weights_y_d, weights_z_d, Rec_d,  Rec_x_d, Rec_y_d,  Rec_y2_d, dx, dy,dz, dt,eta_k1, eta_k2,eta_k3, vrho_x_11, vrho_y_11,vrho_z_11, vrho_x_12,vrho_y_12, vrho_z_12, vrho_x_22,vrho_y_22, vrho_z_22, nx, ny, nz, chi, it);
        cudaDeviceSynchronize();
